<template>
  <div
    class="grid grid-cols-4 gap-2 w-full max-w-md flex-1 min-h-0"
    style="grid-auto-flow: column; grid-template-rows: repeat(10, 1fr)"
  >
    <button
      v-for="n in 40"
      :key="n"
      class="w-full rounded-md text-xl font-semibold transition-all duration-200 flex items-center justify-center shadow-sm active:scale-95 focus:outline-none focus:ring-2 focus:ring-[var(--tg-theme-button-color)]"
      :class="[
        store.isSelected(n)
